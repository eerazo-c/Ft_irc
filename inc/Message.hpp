...
#define ERR_CHANOPRIVSNEEDED(nick) "482 " +  (nick) +  (channel) + ":You're not channel operator"
#define ERR_USERSDONTMATCH(nick) "502 " + (nick) + ":Cant change mode for other users" 
#define ERR_INVALIDMODEPARAM() "696 " + (client) + (target) + (modechar) + (parameter) + ":<description>"
